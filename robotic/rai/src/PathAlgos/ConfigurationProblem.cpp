...
shared_ptr<QueryResult> ConfigurationProblem::query(const arr& x, const rai::String& robot) {
  if(limits.N) {
    for(uint i=0; i<x.N; i++) {
      if(limits(1, i)>limits(0, i) && (x.elem(i)<limits(0, i) || x.elem(i)>limits(1, i))) {
        //LOG(-1) <<"QUERY OUT OF LIMIT: joint " <<i <<": " <<x.elem(i) <<' ' <<limits[i];
  ...
  C.getFrame(robot)->setJointState(x);
  ...
  if(!computeCollisionFeatures) {
    ...
    for(const rai::Proxy& p:C.proxies) {
      ...
      for(const auto& [robot_name, jointMask] : robots) {
        rai::Frame* robot_frame = C.getFrame(robot_name);
        ...
        cout << "INVOLVES ROBOT: " << robot_name << " " << involves_robot << endl;
        if(involves_robot) {
          ...
          if(qr->coll_y_robots.find(robot_name) == qr->coll_y_robots.end()) {
            qr->coll_y_robots[robot_name] = qr->coll_y[i].copy();
            qr->coll_J_robots[robot_name] = qr->coll_J[i].copy();
          } else {
            // Keep the collision with minimum distance
            if(qr->coll_y[i](0) < qr->coll_y_robots[robot_name](0)) {
              qr->coll_y_robots[robot_name] = qr->coll_y[i].copy();
              qr->coll_J_robots[robot_name] = qr->coll_J[i].copy();
            }
          }
